...
export default function ProviderCard({ p }: { p: Provider }) {
  ...
  return (
    <div className={['rounded-2xl p-5 transition', t.card, t.cardHover, 'hover:-translate-y-0.5'].join(' ')}>
      <div className="flex items-center gap-3">
        ...
        <div className="ml-auto text-right">
          <div className="text-sm">{p.rating ? `${p.rating.toFixed(1)} ★` : '—'}</div>
          <div className={`text-xs ${t.mutedText}`}>{p.verified ? 'Verified' : ''}</div>
          <div className={`text-xs ${t.mutedText}`}>{p.distance != null ? `${p.distance.toFixed(1)} mi` : '— mi'}</div>
        </div>
      </div>
